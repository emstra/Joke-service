...
const getIndex = (request, response) => {
    response.writeHead(200, {'Content-Type': 'text/html','Access-Control-Allow-Origin': '*'}); // return access code of 200 (OK)
    response.write(indexPage); // get content of webpage
    response.end();
};

const getStaleJoke = (request, response) => {
    response.writeHead(200, { 'Content-Type' : 'application/json', 'Access-Control-Allow-Origin' : '*'}); // return access code of 200 (OK)
    const joke = jokes[0];
    const json = JSON.stringify(joke);
    response.write(json); // get content of webpage
    response.end();
};
